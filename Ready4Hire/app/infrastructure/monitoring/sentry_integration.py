...
def init_sentry(
    ...
    integrations = [
        ...
        LoggingIntegration(
            level=logging.INFO, event_level=logging.ERROR  # Capturar desde INFO  # Crear eventos desde ERROR
        ),
    ]
    ...
    logger.info(f"✅ Sentry initialized: env={env}, " f"traces={traces_sample_rate}, profiles={profiles_sample_rate}")
...
def capture_exception(error: Exception, level: str = "error", extra: Optional[dict] = None) -> Optional[str]:
    """
    ...
    def sync_wrapper(*args, **kwargs):
        with sentry_sdk.start_transaction(op="function", name=func.__name__):
            try:
                ...
                capture_exception(
                    e, extra={"function": func.__name__, "args": str(args)[:200], "kwargs": str(kwargs)[:200]}
                )
                raise
