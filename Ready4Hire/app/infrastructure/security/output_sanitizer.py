...
class OutputSanitizer:
    ...
    def __init__(self, strict_mode: bool = False, log_violations: bool = True, block_urls: bool = False):
        """
    ...
    def sanitize(self, text: str) -> str:
        ...
        if len(text) > self.MAX_OUTPUT_LENGTH:
            self._log_violation(f"Output demasiado largo: {len(text)} > {self.MAX_OUTPUT_LENGTH}", "length_exceeded")
        ...
        for word in self.BLOCKED_WORDS:
            if word.lower() in text.lower():
                ...
                text = re.sub(rf"\b{re.escape(word)}\b", "[BLOQUEADO]", text, flags=re.IGNORECASE)
...
def get_output_sanitizer(
    strict_mode: bool = False, log_violations: bool = True, block_urls: bool = False
) -> OutputSanitizer:
    ...
    if _output_sanitizer is None:
        _output_sanitizer = OutputSanitizer(
            strict_mode=strict_mode, log_violations=log_violations, block_urls=block_urls
        )
