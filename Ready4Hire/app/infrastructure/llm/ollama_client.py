...
class OllamaClient:
            ...
            for attempt in range(self.max_retries):
                try:
                    ...
                    if attempt < self.max_retries - 1:
                        ...
                        logger.warning(
                            f"Timeout en intento {attempt + 1}/{self.max_retries}. " f"Reintentando en {delay}s..."
                        )
                        time.sleep(delay)
                    else:
                    ...
                    if attempt < self.max_retries - 1:
                        delay = self.retry_delay * (2**attempt)
                        logger.warning(
                            f"Error de conexión en intento {attempt + 1}/{self.max_retries}. "
        ...
        if self.circuit_breaker_enabled and self.circuit_breaker:
            try:
                return self.circuit_breaker.call(
                    self._generate_internal, prompt, model, system, temperature, max_tokens, stream, **kwargs
                )
            ...
            return self._generate_internal(prompt, model, system, temperature, max_tokens, stream, **kwargs)
        ...
        **kwargs,
        ...
        try:
            response = self.session.post(
                f"{self.base_url}/api/generate", json=payload, timeout=self.timeout, stream=stream
            )
            ...
            if stream:
                ...
                self.metrics["avg_latency"] = self.metrics["total_latency"] / self.metrics["successful_requests"]
    ...
    def pull_model(self, model: str) -> bool:
        ...
        try:
            ...
            response = self.session.post(
                f"{self.base_url}/api/pull", json={"name": model}, timeout=600  # 10 minutos para descarga
            )
