...
class AsyncOllamaClient:
    ...
    def __init__(
        ...
        if pool_limits is None:
            pool_limits = httpx.Limits(max_keepalive_connections=20, max_connections=50, keepalive_expiry=30.0)
        ...
        if circuit_breaker_enabled:
            self.circuit_breaker = CircuitBreaker(
                failure_threshold=5, recovery_timeout=60, expected_exception=Exception, name="async_ollama_client"
            )
        else:
    ...
    async def generate(
        ...
        **kwargs,
        ...
        if self.circuit_breaker_enabled and self.circuit_breaker:
            ...
            return await self._generate_internal(prompt, model, system, temperature, max_tokens, **kwargs)

    def _generate_internal_sync(
        self, prompt: str, model: Optional[str], system: Optional[str], temperature: float, max_tokens: int, **kwargs
    ):
