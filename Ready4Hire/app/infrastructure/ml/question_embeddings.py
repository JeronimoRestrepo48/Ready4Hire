...
class QuestionEmbeddingsService:
    ...
    def __init__(self, model_name: str = "all-MiniLM-L6-v2", ranknet_model_path: Optional[str] = None):
        """
    ...
    def _ensure_initialized(self):
        ...
        if not _SENTENCE_TRANSFORMERS_AVAILABLE:
            raise RuntimeError(
                "sentence-transformers no está disponible. " "Instala con: pip install sentence-transformers"
            )
    ...
    def _load_ranknet(self):
        ...
        try:
            ...
            self.ranknet_model.load_state_dict(torch.load(self.ranknet_model_path, map_location="cpu"))
    ...
    def find_similar(
        self, query: str, candidates: List[Dict], top_k: int = 5, text_field: str = "question"
    ...
    def rank_with_ranknet(
        self, user_context: str, candidates: List[Dict], text_field: str = "question"
    ) -> List[Tuple[Dict, float]]:
        """
        ...
        try:
            ...
            features = np.array([np.concatenate([context_emb, q_emb]) for q_emb in candidate_embs])

            # Predecir scores
            with torch.no_grad():
                scores = self.ranknet_model(torch.tensor(features, dtype=torch.float32)).numpy().flatten()

            # Ordenar por score
            top_indices = np.argsort(scores)[::-1]

            results = [(candidates[i], float(scores[i])) for i in top_indices]
    ...
    def cluster_questions(
        self, questions: List[Dict], text_field: str = "question", min_cluster_size: int = 3
    ) -> Dict[int, List[Dict]]:
        """
...
def get_embeddings_service(
    model_name: str = "all-MiniLM-L6-v2", ranknet_model_path: Optional[str] = None
