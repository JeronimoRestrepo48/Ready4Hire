...
class AdvancedQuestionClusteringService:
    ...
    def select_diversified_questions(
        ...
        for cid, questions in self.clusters.items():
            ...
            available = [
                ...
                if q.get('id') in candidate_ids and q.get('id') not in exclude_ids
            ]
            if available:
        ...
        distribution = self._compute_optimal_distribution(
            n_questions,
            available_by_cluster,
            user_profile
        )
    ...
    def _compute_optimal_distribution(
        self,
        n_questions: int,
        available_by_cluster: Dict[int, List[Dict]],
    ...
    def get_cluster_summary(self) -> Dict[str, Any]:
        ...
        for cid, questions in sorted(self.clusters.items()):
            ...
            cluster_info = {
                'id': cid,
                'size': len(questions),
                'topic': self.cluster_metadata.get(cid, ClusterMetadata(
                    cluster_id=cid, size=0, centroid=np.array([]), keywords=[],
                    avg_difficulty=0, topic_label="Unknown", coherence_score=0
                )).topic_label,
                'keywords': self.cluster_metadata.get(cid, ClusterMetadata(
                    cluster_id=cid, size=0, centroid=np.array([]), keywords=[],
                    avg_difficulty=0, topic_label="Unknown", coherence_score=0
                )).keywords,
                'avg_difficulty': self.cluster_metadata.get(cid, ClusterMetadata(
                    cluster_id=cid, size=0, centroid=np.array([]), keywords=[],
                    avg_difficulty=0, topic_label="Unknown", coherence_score=0
                )).avg_difficulty,
                'coherence': self.cluster_metadata.get(cid, ClusterMetadata(
                    cluster_id=cid, size=0, centroid=np.array([]), keywords=[],
                    avg_difficulty=0, topic_label="Unknown", coherence_score=0
                )).coherence_score
            }
