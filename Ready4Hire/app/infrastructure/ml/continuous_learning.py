...
class ContinuousLearningSystem:
    ...
    def __init__(
        ...
        self.strategy_metrics: Dict[str, Dict] = defaultdict(lambda: {
            'selections': 0,
    ...
    def analyze_question_pool(self) -> Dict[str, Any]:
        ...
        analysis = {
            ...
            'avg_questions_per_interview': np.mean([
                len(f.questions_asked) for f in self.feedback_history
            ...
            'most_used_questions': sorted(
                [(p.question_id, p.times_asked) for p in performances],
                key=lambda x: x[1],
                reverse=True
            )[:10],
            
            'least_used_questions': sorted(
                [(p.question_id, p.times_asked) for p in performances],
                key=lambda x: x[1]
            )[:10],
            ...
            'top_performing_questions': sorted(
                [(p.question_id, p.avg_score) for p in performances],
                key=lambda x: x[1],
                reverse=True
            )[:10],
            ...
            if perf_file.exists():
                with open(perf_file, 'r') as f:
                    ...
                    self.question_performance = {
                        q_id: QuestionPerformance(**perf_data)
                        for q_id, perf_data in data.items()
                    }
            ...
            if feedback_file.exists():
                with open(feedback_file, 'r') as f:
                    ...
                    self.feedback_history = deque(
                        [InterviewFeedback(**fb) for fb in data],
                        maxlen=1000
                    )
