...
class LanguageService:
    ...
    def detect(self, text: str) -> Tuple[str, float]:
        ...
        if not self._available:
            logger.warning(
                "⚠️ langid no disponible, usando idioma por defecto"
            )
    ...
    def get_term(
        ...
        if lang not in GLOBAL_TERMS:
            logger.warning(
                f"⚠️ Idioma '{lang}' no soportado, usando '{self.default_language}'"
            )
        ...
        if term_key not in terms:
            logger.warning(
                f"⚠️ Término '{term_key}' no encontrado para idioma '{lang}'"
            )
            return term_key
