...
class EnhancedQuestionSelectorService:
    ...
    async def select_next_question(
        self, interview: Interview, previous_question_ids: List[str], last_answer_score: Optional[float] = None
    ) -> Optional[Question]:
        """
        ...
        try:
            ...
            target_difficulty = self._determine_target_difficulty(
                interview=interview, last_answer_score=last_answer_score
            )

            logger.debug(f"Target difficulty para {interview.id}: {target_difficulty.value}")
            ...
            if not candidates:
                ...
                candidates = await self._get_fallback_questions(
                    role=interview.role, category=interview.interview_type, previous_ids=previous_question_ids
                )
            ...
            selected = await self._select_best_question_ml(
                candidates=candidates, interview=interview, previous_ids=previous_question_ids
            )
    ...
    async def _select_best_question_ml(
        self, candidates: List[Question], interview: Interview, previous_ids: List[str]
    ) -> Question:
        """
        ...
        if self.config.use_clustering and self.clustering_service:
            selected = await self._select_with_clustering(candidates=candidates, previous_ids=previous_ids)
            if selected:
                return selected

        # Estrategia 2: Continuous Learning (si habilitado)
        if self.config.use_continuous_learning and self.learning_system:
            selected = self._select_with_mab(candidates=candidates, interview=interview)
            if selected:
                return selected

        # Estrategia 3: Semantic Diversity (si embeddings disponible)
        if self.config.use_embeddings and self.embeddings_service:
            selected = await self._select_by_semantic_diversity(candidates=candidates, previous_ids=previous_ids)
            if selected:
                return selected
    ...
    async def _select_with_clustering(self, candidates: List[Question], previous_ids: List[str]) -> Optional[Question]:
        """
        ...
        try:
            ...
            visited_clusters = self._get_visited_clusters(previous_ids=previous_ids, all_questions=candidates)
            ...
            if selected_questions and len(selected_questions) > 0:
                ...
                for question in candidates:
                    if question.id == selected_id:
                        logger.info(f"Seleccionada pregunta de cluster {target_cluster} " f"(visits={min_visits})")
                        return question
    ...
    def _select_with_mab(self, candidates: List[Question], interview: Interview) -> Optional[Question]:
        """
        ...
        try:
            ...
            rankings = self.learning_system.get_question_rankings(
                question_ids=candidate_ids, strategy=self.config.exploration_strategy
            )
    ...
    async def _select_by_semantic_diversity(
        self, candidates: List[Question], previous_ids: List[str]
    ) -> Optional[Question]:
        """
        ...
        try:
            ...
            if best_question:
                logger.info(f"Semantic diversity seleccionó {best_question.id} " f"(diversity={max_diversity:.3f})")
    ...
    def _get_visited_clusters(self, previous_ids: List[str], all_questions: List[Question]) -> Dict[int, int]:
        """
    ...
    def _determine_target_difficulty(self, interview: Interview, last_answer_score: Optional[float]) -> SkillLevel:
        """
    ...
    async def _get_candidate_questions(
        self, role: str, category: Optional[str], difficulty: SkillLevel, previous_ids: List[str]
    ) -> List[Question]:
        ...
        candidates = [
            ...
            if (category is None or q.category == category) and q.difficulty == difficulty and q.id not in previous_ids
        ]

        return candidates

    async def _get_fallback_questions(
        self, role: str, category: Optional[str], previous_ids: List[str]
    ) -> List[Question]:
        """Obtiene preguntas de respaldo sin filtro de dificultad."""
        all_questions = await self.question_repo.find_by_role(role)

        candidates = [
            q for q in all_questions if (category is None or q.category == category) and q.id not in previous_ids
        ]

        return candidates

    async def update_question_performance(
        self, interview: Interview, question_id: str, answer_score: float, time_taken: int
    ) -> None:
        """
        ...
        try:
            ...
            logger.info(f"Performance actualizada para {question_id}: " f"score={answer_score}, time={time_taken}s")
    ...
    async def get_selection_metrics(self) -> Dict[str, Any]:
        ...
        metrics = {
            ...
            "cache": {"questions_cached": len(self._question_cache), "clusters_cached": len(self._cluster_cache)},
        }
