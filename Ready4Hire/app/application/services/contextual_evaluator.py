...
class InterviewHistoryAnalyzer:
    ...
    def _identify_concept_patterns(
        self,
        answers: List[Answer]
...
class ContextualEvaluator:
    ...
    def _enrich_with_context(
        ...
        if context["performance_trend"] == "improving":
            ...
            enriched["strengths"].insert(
                0,
                "🎯 Progresión notable: tu rendimiento está mejorando consistentemente"
            )
        elif context["performance_trend"] == "declining":
            score_adjustment = -0.5
            enriched["improvements"].insert(
                0,
                "⚠️ Atención: tu rendimiento está declinando. Considera tomar un descanso o revisar conceptos básicos"
            )
        ...
        if improved_concepts:
            enriched["strengths"].append(
                f"✨ Mejora notable en: {', '.join(list(improved_concepts)[:2])} "
                f"(anteriormente eran áreas débiles)"
            )
    ...
    def get_interview_summary(self, interview: Interview) -> Dict[str, Any]:
        ...
        if not context["has_sufficient_context"]:
            return {
                ...
                "message": "Se necesitan al menos 3 respuestas para generar resumen"
            }
