...
def transcribe_audio(file: UploadFile, lang: str = 'es') -> str:
    ...
    if not mime or not mime.startswith('audio'):
        raise ValueError('El archivo no es de audio válido.')
    ...
    try:
        ...
        try:
            ...
            print(f"[WARN] No se pudo borrar el temporal de audio: {e}")
    return text
...
def synthesize_text(text: str, lang: str = 'es', out_path: Optional[str] = None) -> str:
    ...
    try:
        ...
        try:
            ...
            print(f"[ERROR] TTS: {e}")
            raise RuntimeError('No se pudo sintetizar el audio.')
        return out_path
    finally:
        try:
            engine.stop()
